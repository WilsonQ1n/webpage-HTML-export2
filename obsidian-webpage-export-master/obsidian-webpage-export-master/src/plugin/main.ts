...
export default class HTMLExportPlugin extends Plugin {
	...
	async onload() {
		console.log("Loading webpage-html-export2-wilson plugin");
		try {
			...
			// Register the settings tab first so the gear icon remains available
			// even when later initialization hits an error.
			...
			console.error("Failed to initialize webpage-html-export2-wilson plugin:", error);
			new Notice("Webpage HTML Export2 初始化失败，请打开开发者控制台查看错误。", 8000);
		}
	}
	...
	onunload() {
		ExportLog.log("unloading webpage-html-export2-wilson plugin");
	}
}
