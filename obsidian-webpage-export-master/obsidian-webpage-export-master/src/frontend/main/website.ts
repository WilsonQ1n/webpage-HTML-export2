...
export class ObsidianWebsite {
	...
	private outlineSourceDocument: ObsidianDocument | null = null;
	...
	private async onInit() {
		...
		this.outlineSourceDocument = this.document;
	...
	private replaceOutlineElement(newOutlineEl?: HTMLElement | null) {
		const currentOutlineEl = document.querySelector("#outline") as HTMLElement | null;
		if (!currentOutlineEl) return;

		let outlineEl: HTMLElement;
		if (newOutlineEl) {
			outlineEl = document.adoptNode(newOutlineEl);
		} else {
			const header = currentOutlineEl.querySelector(".feature-header")?.cloneNode(true) as HTMLElement | null;
			outlineEl = document.createElement("div");
			outlineEl.id = "outline";
			outlineEl.className = "tree-container";
			if (header) outlineEl.appendChild(header);
		}

		currentOutlineEl.replaceWith(outlineEl);
		this.outlineTree = new Tree(outlineEl, this.metadata.featureOptions.outline.minCollapseDepth);
	}

	public setOutlineForDocument(doc?: ObsidianDocument, syncScroll: boolean = true) {
		const outlineEl = doc?.sourceHtml?.querySelector("#outline") as HTMLElement | null;
		this.replaceOutlineElement(outlineEl);
		this.outlineSourceDocument = doc ?? null;
		if (syncScroll && doc) {
			this.initializeOutlineScrollSync();
		} else {
	...
	public async loadURL(url: string, pushState: boolean = true): Promise<ObsidianDocument | undefined> {
		...
		this.setOutlineForDocument(page, false);
	...
	private initializeOutlineScrollSync() {
		this.teardownOutlineScrollSync();
		const sourceDoc = this.outlineSourceDocument ?? this.document;
		if (!this.outlineTree || !sourceDoc?.documentEl) return;

		const headings = Array.from(
			sourceDoc.documentEl.querySelectorAll("h1[id], h2[id], h3[id], h4[id], h5[id], h6[id]")
		) as HTMLElement[];
		if (headings.length == 0) return;

		this.outlineHeadingEls = headings;
		this.outlineScrollContainer = sourceDoc.documentEl;
	...
	private findOutlineItemByHeaderId(headerId: string): TreeItem | undefined {
		...
		const sourceDoc = this.outlineSourceDocument ?? this.document;
		const pathname = sourceDoc?.pathname ?? "";
