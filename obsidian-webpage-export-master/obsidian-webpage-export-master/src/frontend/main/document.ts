...
export class WebpageDocument {
	...
	public postProcess() {
		...
		this.replaceExternalEmbeds();
	}

	private replaceExternalEmbeds() {
		if (!this.documentEl) return;

		const isExternalSrc = (src: string) => /^https?:/i.test(src);
		...
		const webviews = Array.from(this.documentEl.querySelectorAll("webview")) as HTMLElement[];
		...
		const iframeEls = Array.from(this.documentEl.querySelectorAll("iframe")) as HTMLElement[];
		for (const iframe of iframeEls) {
			const src = iframe.getAttribute("src");
			if (!src || !isExternalSrc(src)) continue;
			if (iframe.getAttribute("data-external-embed") === "true") continue;
			iframe.setAttribute("data-external-embed", "true");

			const fallback = document.createElement("div");
			fallback.className = "external-embed-fallback";

			const title = document.createElement("div");
			title.className = "external-embed-title";
			title.textContent = "\u5916\u90E8\u7F51\u9875\u8BF7\u5728\u6D4F\u89C8\u5668\u6253\u5F00";
			...
			const width = normalizeSize(iframe.getAttribute("width") || (iframe as HTMLElement).style.width);
			const height = normalizeSize(iframe.getAttribute("height") || (iframe as HTMLElement).style.height);
