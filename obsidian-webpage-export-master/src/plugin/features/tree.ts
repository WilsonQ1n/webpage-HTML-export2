...
export class TreeItem
{
		...
		else
		{
			...
			if (this.collapseIcon)
			{
				if (this.children.length == 0)
					this.collapseIcon.style.opacity = "0";
				else
					this.collapseIcon.style.opacity = "1";
			}
	...
	{
		return (this.children.length != 0 || this.depth == 1) && this.depth >= this.minCollapsableDepth;
	}
	...
	protected async insertSelf(container: HTMLElement): Promise<HTMLElement>
	{
		...
		if (this.isCollapsible())
		{
			...
			// 初始渲染时检查是否有子项，如果没有子项，将展开符号设为透明
			if (this.collapseIcon && this.children.length == 0)
				this.collapseIcon.style.opacity = "0";
		}
