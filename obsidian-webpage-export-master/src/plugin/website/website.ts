...
export class Website
{
	...
	public async build(files?: TFile[]): Promise<Website | undefined>
	{
		...
		// Refresh asset downloads that may be discovered during rendering (e.g., MathJax fonts).
		const lateDownloads = AssetHandler.getDownloads(this.destination, this.exportOptions)
			.filter((asset) => !this.index.allFiles.includes(asset));
		if (lateDownloads.length > 0)
		{
			await this.index.addFiles(lateDownloads);
		}
	...
	public getFilePathFromSrc(src: string, exportingFilePath: string): Path
	{
		...
		{
			// Split src into path, query, and hash parts
			const querySplit = src.split("?");
			const hashSplit = querySplit[0].split("#");
			const path = hashSplit[0];
			const hash = hashSplit[1]?.trim();
			const query = querySplit[1]?.trim();
			
			// Try to resolve the path using Obsidian's metadata cache
			const resolvedPath = app.metadataCache.getFirstLinkpathDest(path, exportingFilePath)?.path;
			
			if (resolvedPath) {
				// If path is resolved, use it
				...
				// If path is not resolved, keep the original src
				// This ensures relative paths with query parameters are preserved
				pathString = src;
			}
		}
