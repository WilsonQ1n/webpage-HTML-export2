...
export namespace _MarkdownRendererInternal {
	...
	async function renderMarkdownViewFallback(preview: MarkdownPreviewView, options: MarkdownRendererOptions): Promise<HTMLElement | undefined> {
		...
		for (const section of sections) {
			...
			await waitUntil(() => !section.el.querySelector("[class^='block-language-']:empty") && !section.el.querySelector(".tabs-container:empty") || checkCancelled(), 500, 1);
			if (checkCancelled()) return undefined;
	...
	export async function renderMarkdownView(preview: MarkdownPreviewView, options: MarkdownRendererOptions): Promise<HTMLElement | undefined> {
		...
		const getSections = (): RenderSection[] => (renderer.sections ?? []) as RenderSection[];

		const countUnrenderedSections = (sectionList: RenderSection[]): number =>
		{
			let unrendered = 0;
			for (const section of sectionList)
			{
				const expectedHtml = (section.html ?? "").trim();
				if (expectedHtml.length == 0) continue;
				...
				const actualHtml = section.el?.innerHTML?.trim() ?? "";
				if (!section.el || actualHtml.length == 0)
				{
					unrendered++;
				}
			}
		...
		const renderedSectionsSuccess = await waitUntil(
			() => countUnrenderedSections(getSections()) == 0 || checkCancelled(),
			2000,
			50
		);
		if (checkCancelled()) return undefined;
		if (!renderedSectionsSuccess)
		{
			const sections = getSections();
			const unrenderedCount = countUnrenderedSections(sections);
			ExportLog.log(
				`Detected ${unrenderedCount} unrendered sections in file ${preview.file.name}, using fallback!`
			);
			return renderMarkdownViewFallback(preview, options);
		}
		...
		const invalidPluginBlocks = Array.from(
			preview.containerEl.querySelectorAll(
				"[class^='block-language-']:empty, .tabs-container:empty"
			)
		);
		for (const block of invalidPluginBlocks) {
			ExportLog.warning(
	...
	export async function postProcessHTML(html: HTMLElement, options: MarkdownRendererOptions) {
		...
		// fix model-viewer elements by setting src attribute from parent embed data
		html.querySelectorAll(".model-viewer-embed").forEach((embedEl: HTMLElement) => {
			const modelViewer = embedEl.querySelector("model-viewer");
			if (modelViewer) {
				// Get the src from the parent embed element's data
				const src = embedEl.getAttribute("src");
				if (src) {
					modelViewer.setAttribute("src", src);
					modelViewer.classList.remove("is-unresolved");
				}
			}
		});
