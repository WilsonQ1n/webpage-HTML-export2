...
export class Index
{
	...
	private minisearchOptions = 
	{
		...
		// Prevent internal auto-vacuum crashes during large batch updates.
		autoVacuum: false
	}
	...
	private async addWebpageToMinisearch(webpage: Webpage)
	{
		...
		const addDoc = async () => {
			...
			if (this.minisearch && this.minisearch.has(webpagePath)) {
		...
		try {
			...
			ExportLog.warning(error, "Minisearch update failed. Rebuilding index.");
			try {
				this.minisearch = new Minisearch(this.minisearchOptions);
				await addDoc();
			} catch (error2) {
				ExportLog.warning(error2, "Minisearch rebuild failed. Search index will be incomplete for this export.");
			}
		}
	}
